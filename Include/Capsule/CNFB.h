...
typedef enum clCNFBCallResult {
  ...
  clC_NFB_ModelIsNotTrained, /**< Model was not trained, must be trained
                                first. */
  clC_NFB_ModelIsTrained,    /**< Model is already trained, does not need any
                                training. */
 ...
 * clCNFB_IsModelTrained will return \c false if it needs to be trained.
 *
...
CL_DLL void
clCNFBDelegateNFBUserState_Set(clCNFBDelegateNFBUserState delegate,
...
CL_DLL void clCNFBDelegate_Set(clCNFBDelegate delegate,
                               clCNFBHandler callback) NOEXCEPT;
...
CL_DLL clCNFBDelegateNFBUserState
clCNFB_GetOnUserStateChangedEvent(clCNFB nfb) NOEXCEPT;
 ...
 * \returns call result.
 */
CL_DLL clCNFBCallResult
clCNFB_InitializeWithPeriod(clCNFB nfb, float userStatePeriod) NOEXCEPT;
/**
 ...
 */
CL_DLL clCNFBCallResult
clCNFB_AddFeedbackFunction(clCNFB nfb, const char *feedbackFunction) NOEXCEPT;

/**
 * \brief Request training, event is called when training is finished
 *
...
/**
 * \brief Request reset, event is called when reset finished
 *
...
/**
 * \brief Check whether model was trained.
 *
 * \param nfb NFB handle
 *
 * \returns whether model is trained.
 */
 ...
 *
 * \param calibrator NFB calibrator handle. A calibrated calibrator must be
 * passed. \returns NFB handle.
 */
CL_DLL clCNFB clCNFB_CreateCalibrated(clCNFBCalibrator calibrator,
                                      clCError *error) NOEXCEPT;

/**
 * \brief Mark a change in user state
 *
 * Used in BCI system to train classifier to discriminate states.
 * Should always end with Undefined state before Train() to mark an ending.
 *
 * \param nfb NFB handle
 * \param userState state to be marked
 *
 * \returns call result.
 */
// DEPRECATED
CL_DLL clCNFBCallResult clCNFB_MarkState(clCNFB nfb,
                                         ccCNFBState userState) NOEXCEPT;
