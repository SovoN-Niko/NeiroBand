...
#define PLATFORM_ADDRES "https://productivity.preprod.neiry-bci.com"
...
void onEmotionUpdate(clCEmotions emotion, const clCEmotionalStates *emotions) {
  logger.log(INFO, "Emotion update:\n");
  std::stringstream line_stream;
  line_stream << emotions->focus << " " << emotions->stress
              << " " << emotions->chill;
...
void onCalibratedCallback(clCEmotions emotion) {
  logger.log(INFO, "Emotion calibrated ");
}

void onInitializedCallback(clCEmotions emotion) {
  logger.log(INFO, "Emotion initialized ");
}

void onCalibrateFinished(clCNFBCalibrator) {
  logger.log(INFO, "NFB calibrated ");
}
...
void onSessionStarted(clCSession session) {
  ...
  clCNFBCalibratorDelegateIndividualNFBCalibrationStage
      onCalibrationFinishedStageEvent =
          clCNFBCalibrator_GetOnIndividualNFBStageFinishedEvent(calibrator);

  clCNFBCalibratorDelegateIndividualNFBCalibrationStage_Set(
      onCalibrationFinishedStageEvent, onCalibrateFinished);
  clCError error_cal = clC_Error_OK;
  ...
  clCEmotions emotions = clCEmotions_Create(session, 0.05, 0.05);
  clCEmotions_Initialize(emotions, PLATFORM_ADDRES);

  clCEmotionsDelegate initEmotionsDelegate =
      clCEmotions_GetOnInitializedEvent(emotions);

  clCEmotionsDelegate calibrateEmotionsDelegate =
      clCEmotions_GetOnCalibratedEvent(emotions);

  clCEmotionsDelegateEmotionalStatesUpdate updateDelegate =
      clCEmotions_GetOnEmotionalStatesUpdateEvent(emotions);

  clCEmotionsDelegateEmotionalStatesUpdate_Set(updateDelegate, onEmotionUpdate);

  clCEmotionsDelegate_Set(initEmotionsDelegate, onInitializedCallback);
  clCEmotionsDelegate_Set(calibrateEmotionsDelegate, onCalibratedCallback);
